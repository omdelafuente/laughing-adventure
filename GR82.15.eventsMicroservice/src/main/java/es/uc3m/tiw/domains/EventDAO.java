...
public interface EventDAO extends CrudRepository<Event, Integer> {
	...
	@Query("SELECT c FROM Event c WHERE (:title IS NULL OR :title IS '' OR c.title LIKE CONCAT('%',:title,'%')) AND (:category IS NULL OR :category IS '' OR c.category = :category) AND (:place IS NULL OR :place IS ''  OR c.place LIKE CONCAT('%',:place,'%')) AND (:description IS NULL OR :description IS '' OR c.description LIKE CONCAT('%',:description,'%')) AND (:state IS NULL OR :state IS '' OR c.state = :state) AND (:priceMin IS NULL OR c.price >= :priceMin) AND (:priceMax IS NULL OR c.price <= :priceMax) AND (:dateMin IS NULL OR c.eventDate >= :dateMin) AND (:dateMax IS NULL OR c.eventDate <= :dateMax)")
